...
# Masukkan API Key dari Google Cloud Console lo di sini
...
def scrape_toko_bangunan(kecamatan):
    ...
    # Ambil maksimal 8 toko teratas per kecamatan
...
for kec in kecamatan_list:
    print(f"--- Data Toko Bangunan di {kec} ---")
    data_toko = scrape_toko_bangunan(kec)
    
    for idx, toko in enumerate(data_toko, 1):
        print(f"{idx}. {toko['Nama']}")
        print(f"   Alamat: {toko['Alamat']}")
        print(f"   Rating: {toko['Rating']}\n")
        
    # Kasih jeda waktu supaya nggak kena limit/block dari Google
    time.sleep(2)